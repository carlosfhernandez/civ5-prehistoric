...
function UpdateData()
	...
	if( iPlayerID >= 0 ) then
		...
		if (pPlayer:GetNumCities() > 0) then
			...
			-------------------------------------------------------------------------
			local strSocietyStr;
			local iSociety = pPlayer:GetCivSocietyPoints()
			if iSociety < 0 then
				strHealthStr = string.format("[ICON_SOCIETY][COLOR:255:77:105:255]%i%%[/COLOR]", iSociety);
			else
				strHealthStr = string.format("[ICON_SOCIETY][COLOR:255:77:105:255]+%i%%[/COLOR]", iSociety);
			end
			Controls.SocietyString:SetText(strSocietyStr);
			--end addition
