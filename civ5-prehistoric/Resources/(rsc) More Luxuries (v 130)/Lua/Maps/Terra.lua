...
function AssignStartingPlots:GetDisabledLuxuriesTargetNumber()
	...
	-- MOD: Updated -- original values were 5, 3, 1, 0, 0, 0 when there was only 20 total luxuries.
	-- MOD: Total luxury numbers are similar to the original ones, though, maps get more and more as they increase from Standard, but these maps are immense and needed more luxuries anyway!
	-- MOD: Orginal totals: 15, 17, 19, 20, 20, 20
	-- MOD:     New totals: 15, 17, 19, 21, 23, 25
	local worldsizes = {								-- MOD: Using the default values and keeping things the same:
		[GameInfo.Worlds.WORLDSIZE_DUEL.ID] = 13,		-- MOD: 28 - ( 2 regional + 1 marble + 3 city-state + 9 random) = 13
		[GameInfo.Worlds.WORLDSIZE_TINY.ID] = 11,		-- MOD: 28 - ( 4 regional + 1 marble + 3 city-state + 9 random) = 11
		[GameInfo.Worlds.WORLDSIZE_SMALL.ID] = 9,		-- MOD: 28 - ( 6 regional + 1 marble + 3 city-state + 9 random) =  9
		[GameInfo.Worlds.WORLDSIZE_STANDARD.ID] = 7,	-- MOD: 28 - ( 8 regional + 1 marble + 3 city-state + 9 random) =  7
		[GameInfo.Worlds.WORLDSIZE_LARGE.ID] = 5,		-- MOD: 28 - (10 regional + 1 marble + 3 city-state + 9 random) =  5
		[GameInfo.Worlds.WORLDSIZE_HUGE.ID] = 3,		-- MOD: 28 - (12 regional + 1 marble + 3 city-state + 9 random) =  3
		}
	local maxToDisable = worldsizes[Map.GetWorldSize()];
	return maxToDisable
end
------------------------------------------------------------------------------
-- MOD: New -- will increase random luxuries on all map sizes so luxury totals are close to the original script, and since they're larger than normal (New Standard = Old Large, New Large = Old Huge, etc.)
function AssignStartingPlots:__CustomInit()
	self.iNumTypesRandom = 9
end	
------------------------------------------------------------------------------
