...
function AssignStartingPlots.Create()
	...
	local findStarts = {
		...
		amounts_of_resources_placed = table.fill(0, 75), -- Stores amounts of each resource ID placed. WARNING: This table uses adjusted resource ID (+1) to account for Lua indexing. Add 1 to all IDs to index this table.	-- MOD: Updated -- original = 45 -- any new luxuries added post BNW start at ID 40, so this needs to be increased!
		luxury_assignment_count = table.fill(0, 75), -- Stores amount of each luxury type assigned to regions. WARNING: current implementation will crash if a Luxury is attached to resource ID 0 (default = iron), because this table uses unadjusted resource ID as table index.
		luxury_low_fert_compensation = table.fill(0, 75), -- Stores number of times each resource ID had extras handed out at civ starts. WARNING: Indexed by resource ID.
		...
		iNumTypesUnassigned = 28,		-- Total number of luxuries. Adjust if modifying number of luxury resources.	-- MOD: Updated -- original = 20
		iNumMaxAllowedForRegions = 18,	-- Maximum luxury types allowed to be assigned to regional distribution. CANNOT be reduced below 8!		-- MOD: Updated -- original = 8
		...
		iNumTypesRandom = 6,	-- MOD: Updated -- original = 0
		...
		dry_plains_flat_no_feature = {},			-- MOD: New
		fresh_water_plains_flat_no_feature = {},	-- MOD: New
		tundra_flat_forest = {},					-- MOD: New
		desert_or_tundra_flat_no_feature = {},		-- MOD: New -- This is for Marble.  Instead of adding a quinary index to the system, so it's also placed on flat featureless tundra, I simply created a new "combo" plot list to index.
													-- MOD: 	-- This fix is only for the second luxury type placed at starts.  The special method which places marble randomly throughout the map is fine and already includes all of these per marble_list.
													-- MOD: 	-- Note: grass_flat_no_feature and tundra_flat_no_feature are already initialized below, but I'll include them in the luxury plot lists later on, if modders want to use it.
		...
		-- Mod luxuries
		cocoa_ID, coffee_ID, tea_ID, tobacco_ID, amber_ID, jade_ID, olives_ID, perfume_ID,	-- MOD: New
...
function AssignStartingPlots:__Init()
	...
	for resource_data in GameInfo.Resources() do
		...
		-- Mod Luxury IDs
		elseif resourceType == "RESOURCE_COCOA" then	-- MOD: New
			self.cocoa_ID = resourceID;
		elseif resourceType == "RESOURCE_COFFEE" then	-- MOD: New
			self.coffee_ID = resourceID;
		elseif resourceType == "RESOURCE_TEA" then		-- MOD: New
			self.tea_ID = resourceID;
		elseif resourceType == "RESOURCE_TOBACCO" then	-- MOD: New
			self.tobacco_ID = resourceID;
		elseif resourceType == "RESOURCE_AMBER" then	-- MOD: New
			self.amber_ID = resourceID;
		elseif resourceType == "RESOURCE_JADE" then		-- MOD: New
			self.jade_ID = resourceID;
		elseif resourceType == "RESOURCE_OLIVE" then	-- MOD: New
			self.olives_ID = resourceID;
		elseif resourceType == "RESOURCE_PERFUME" then	-- MOD: New
...
function AssignStartingPlots:__InitLuxuryWeights()
	...
	{self.silver_ID,	25},
	{self.copper_ID,	15},
	{self.amber_ID,		15},		-- MOD: New
	{self.salt_ID,		15},
	{self.gold_ID,		10},		-- MOD: Added
	{self.gems_ID,		05},
	{self.dye_ID,		05},	};

	self.luxury_region_weights[2] = {			-- Jungle
	{self.citrus_ID,	35},
	{self.cocoa_ID,		35},		-- MOD: New
	{self.spices_ID,	30},
	{self.gems_ID,		20},
	{self.sugar_ID,		20},
	{self.pearls_ID,	20},
	--{self.copper_ID,	05},		-- MOD: Disabled -- not really needed here and dilutes the list
	{self.jade_ID,		10},		-- MOD: New
	{self.truffles_ID,	05},
	{self.crab_ID,		05},
	{self.silk_ID,		05},
	{self.dye_ID,		05},	};
	...
	{self.truffles_ID,	30},
	{self.fur_ID,		10},
	{self.spices_ID,	15},		-- MOD: Updated -- original = 10
	{self.citrus_ID,	05},
	{self.cocoa_ID,		05},		-- MOD: New
	{self.amber_ID,		10},		-- MOD: New
	{self.salt_ID,		05},
	--{self.copper_ID,	05},		-- MOD: Disabled -- not really needed here and dilutes the list
	{self.crab_ID,		10},
	{self.whale_ID,		10},
	...
	{self.incense_ID,	35},
	{self.salt_ID,		25},		-- MOD: Updated -- original = 15 -- increased to give this region a better chance for food support
	{self.gold_ID,		25},
	{self.copper_ID,	15},		-- MOD: Updated -- original = 10
	{self.silver_ID,	15},		-- MOD: Added
	{self.cotton_ID,	05},		-- MOD: Updated -- original = 15 -- lessened chance since it's limited to flood plains if no other terrains/features exist in the area besides desert
	{self.sugar_ID,		05},		-- MOD: Updated -- original = 15 -- lessened chance since it's limited to flood plains if no other terrains/features exist in the area besides desert
	{self.pearls_ID,	05},
	{self.whale_ID,		05},		-- MOD: Added -- whales only avoid jungle regions during regional placement
	{self.citrus_ID,	05},	};
	...
	{self.silver_ID,	30},
	{self.copper_ID,	30},
	{self.jade_ID,		30},		-- MOD: New
	{self.amber_ID,		10},		-- MOD: New
	{self.gems_ID,		15},
	{self.pearls_ID,	05},		-- MOD: Updated -- original = 15
	--{self.salt_ID,	10},		-- MOD: Disabled -- not really needed here and dilutes the list
	{self.crab_ID,		15},		-- MOD: Updated -- original = 10 -- hill regions really need more food, makes sense to favor crab and whale for coastal hill region starts
	{self.whale_ID,		15},	};	-- MOD: Updated -- original = 10 -- hill regions really need more food, makes sense to favor crab and whale for coastal hill region starts
	
	self.luxury_region_weights[6] = {			-- Plains
	{self.ivory_ID,		35},
	{self.wine_ID,		35},
	{self.salt_ID,		25},
	{self.olives_ID,	35},		-- MOD: New
	{self.coffee_ID,	10},		-- MOD: New
	{self.tobacco_ID,	15},		-- MOD: New
	{self.tea_ID,		05},		-- MOD: New
	{self.perfume_ID,	05},		-- MOD: New
	{self.incense_ID,	10},
	--{self.spices_ID,	05},		-- MOD: Disabled -- not really needed here and dilutes the list
	{self.whale_ID,		05},
	{self.pearls_ID,	10},		-- MOD: Updated -- original = 05
	{self.crab_ID,		05},
	--{self.truffles_ID,	05},	-- MOD: Disabled -- original = 05 -- there's a chance forests, jungle, or marsh aren't present in a heavy open plains area; this isn't needed here
	{self.gold_ID,		10},	};	-- MOD: Updated -- original = 05
	
	self.luxury_region_weights[7] = {			-- Grass
	{self.cotton_ID,	35},		-- MOD: Updated -- original = 30
	{self.silver_ID,	20},
	{self.sugar_ID,		25},		-- MOD: Updated -- original = 20
	{self.copper_ID,	20},
	{self.crab_ID,		05},		-- MOD: Updated -- original = 20
	{self.tea_ID,		30},		-- MOD: New
	{self.perfume_ID,	30},		-- MOD: New
	{self.olives_ID,	05},		-- MOD: New
	{self.coffee_ID,	15},		-- MOD: New
	{self.tobacco_ID,	10},		-- MOD: New
	{self.pearls_ID,	05},		-- MOD: Updated -- original = 10
	{self.whale_ID,		05},		-- MOD: Updated -- original = 10
	{self.truffles_ID,	05},
	--{self.gems_ID,	05},		-- MOD: Disabled -- not really needed here and dilutes the list
	{self.spices_ID,	05},	};
	...
	{self.citrus_ID,	05},
	{self.cocoa_ID,		05},		-- MOD: New
	{self.coffee_ID,	05},		-- MOD: New
	{self.tobacco_ID,	05},		-- MOD: New
	{self.tea_ID,		05},		-- MOD: New
	{self.jade_ID,		05},		-- MOD: New
	{self.amber_ID,		05},		-- MOD: New
	{self.olives_ID,	05},		-- MOD: New
	{self.perfume_ID,	05},		-- MOD: New
	{self.incense_ID,	05},
	{self.silk_ID,		05},
	{self.gems_ID,		05},
	{self.dye_ID,		05},		-- MOD: Added; was missing
	{self.fur_ID,		05},		-- MOD: Added; was missing
	{self.gold_ID,		05},	};
	
	-- MOD: This list is important, since the regional luxury cap has been dramatically increased.  This list could be utilized when a prominent region type on a map is used by many civs and runs out of regional luxuries on its own list.
	-- MOD: So, I'm going to skim through it and increase luxuries which are very flexible and decrease the chances of luxuries which aren't very flexible.
	self.luxury_fallback_weights = {											-- Fallbacks, in case of extreme map conditions, or
	{self.whale_ID,		20},		-- MOD: Updated -- original = 10			-- for games with oodles of civilizations.
	{self.pearls_ID,	20},		-- MOD: Updated -- original = 10
	{self.gold_ID,		10},
	{self.silver_ID,	10},		-- MOD: Updated -- original = 05			-- This list is also used to assign Disabled and Random types.
	{self.gems_ID,		10},													-- So it's important that this list contain every available luxury type.
	{self.ivory_ID,		05},
	{self.fur_ID,		05},		-- MOD: Updated -- original = 10			-- NOTE: Marble affects Wonders, so is handled as a special case, on the side.
	{self.dye_ID,		05},
	{self.spices_ID,	05},
	{self.silk_ID,		05},
	{self.sugar_ID,		05},
	...
	{self.perfume_ID,	05},		-- MOD: New
	{self.incense_ID,	05},
	{self.copper_ID,	10},		-- MOD: Updated -- original = 05
	{self.salt_ID,		10},		-- MOD: Updated -- original = 05
	{self.citrus_ID,	05},
	{self.truffles_ID,	05},
	{self.crab_ID,		20},	};	-- MOD: Updated -- original = 10
	...
	{self.fur_ID,		15},
	{self.dye_ID,		10},
	{self.cocoa_ID,		10},		-- MOD: New
	{self.coffee_ID,	10},		-- MOD: New
	{self.tobacco_ID,	10},		-- MOD: New
	{self.tea_ID,		10},		-- MOD: New
	{self.jade_ID,		10},		-- MOD: New
	{self.amber_ID,		10},		-- MOD: New
	{self.olives_ID,	10},		-- MOD: New
	{self.perfume_ID,	10},		-- MOD: New
	{self.spices_ID,	15},
	{self.silk_ID,		15},
	{self.sugar_ID,		10},
	{self.cotton_ID,	10},
...
function AssignStartingPlots:GenerateGlobalResourcePlotLists()
	...
	local temp_dry_plains_flat_no_feature, temp_fresh_water_plains_flat_no_feature = {}, {};	-- MOD: New
	local temp_desert_or_tundra_flat_no_feature, temp_tundra_flat_forest = {}, {};				-- MOD: New
	--
	-- local iW, iH = Map.GetGridSize();	-- MOD: Disabled -- already initialized above, though, not a big deal
				...
				elseif plotType == PlotTypes.PLOT_LAND then
					...
					elseif featureType == FeatureTypes.FEATURE_FOREST then
						...
						if terrainType == TerrainTypes.TERRAIN_TUNDRA then
							...
							table.insert(temp_tundra_flat_forest, i);					-- MOD: New
						else
					...
					elseif featureType == FeatureTypes.NO_FEATURE then
						...
						elseif terrainType == TerrainTypes.TERRAIN_TUNDRA then
							...
							table.insert(temp_tundra_flat_including_forests, i);
							table.insert(temp_marble_list, i);
							table.insert(temp_desert_or_tundra_flat_no_feature, i);		-- MOD: New
						elseif terrainType == TerrainTypes.TERRAIN_DESERT then
						...
						elseif terrainType == TerrainTypes.TERRAIN_PLAINS then
							...
							if plot:IsFreshWater() then										-- MOD: Updated fresh water check
								table.insert(temp_fresh_water_plains_flat_no_feature, i);	-- MOD: New
							else
								table.insert(temp_dry_plains_flat_no_feature, i);			-- MOD: New
								table.insert(temp_marble_list, i);
							end
						elseif terrainType == TerrainTypes.TERRAIN_GRASS then
	...
	self.dry_plains_flat_no_feature = GetShuffledCopyOfTable(temp_dry_plains_flat_no_feature)					-- MOD: New
	self.fresh_water_plains_flat_no_feature = GetShuffledCopyOfTable(temp_fresh_water_plains_flat_no_feature)	-- MOD: New
	self.desert_or_tundra_flat_no_feature = GetShuffledCopyOfTable(temp_desert_or_tundra_flat_no_feature)		-- MOD: New
	self.tundra_flat_forest = GetShuffledCopyOfTable(temp_tundra_flat_forest)									-- MOD: New
	--
	...
	self.grass_flat_no_feature,					-- 16	-- MOD: New
	self.tundra_flat_no_feature,				-- 17	-- MOD: New
	self.dry_plains_flat_no_feature,			-- 18	-- MOD: New
	self.fresh_water_plains_flat_no_feature,	-- 19	-- MOD: New
	self.desert_or_tundra_flat_no_feature,		-- 20	-- MOD: New
	self.tundra_flat_forest,					-- 21	-- MOD: New
	};
		...
		for index, resource_options in ipairs(self.luxury_fallback_weights) do
			...
			if self.luxury_assignment_count[res_ID] < 3 then -- This type still eligible.
				...
				if self.iNumTypesAssignedToRegions < self.iNumMaxAllowedForRegions or test == true then -- Won't exceed allowed types.
					if res_ID == self.whale_ID or res_ID == self.pearls_ID or res_ID == self.crab_ID then		-- MOD: Fixed -- added missing crab restriction for regional placement
						if res_ID == self.whale_ID and self.regionTypes[region_number] == 2 then
							-- No whales in jungle regions, sorry
						elseif res_ID == self.pearls_ID and self.regionTypes[region_number] == 1 then
							-- No pearls in tundra regions, sorry
						elseif res_ID == self.crab_ID and self.regionTypes[region_number] == 4 then		-- MOD: Fixed -- added missing crab restriction for regional placement
						-- No crabs in desert regions, sorry	
						else
							if self.startLocationConditions[region_number][1] == true then -- This region's start is along an ocean, so water-based luxuries are allowed.
								if self.regionTerrainCounts[region_number][8] >= 12 then -- Enough water available.
...
function AssignStartingPlots:GetLuxuriesSplitCap()
	...
	local split_cap = 1;
	-- MOD: New -- With a new regional luxury cap of 18, there's no need for a split cap higher than 2 to cover the maximum civ count of 22 (16 x 2 = 32)
	...
	-- MOD: Was going to stick with only 12 here, but I increased it to 18 for Huge players that like a lot more civs.
	if self.iNumCivs > 18 then	
		split_cap = 2
	end
	--[[	MOD: Disabled
...
function AssignStartingPlots:GetDisabledLuxuriesTargetNumber()
	-- This data was separated out to allow easy replacement in map scripts.
	-- MOD: Updated -- original values were 10, 7, 5, 3, 1, 0
	-- MOD: new target numbers keep total luxuries somewhat similar to the original game
	-- MOD: default totals: 10, 13, 15, 17, 19, 20
	-- MOD:     new totals: 10, 13, 15, 17, 19, 22  (Huge really needed more anyway)
	local worldsizes = {								-- MOD: Using the default values and keeping things the same:
		[GameInfo.Worlds.WORLDSIZE_DUEL.ID] = 18,		-- MOD: 28 - ( 2 regional + 1 marble + 3 city-state + 4 random) = 18
		[GameInfo.Worlds.WORLDSIZE_TINY.ID] = 15,		-- MOD: 28 - ( 4 regional + 1 marble + 3 city-state + 5 random) = 15
		[GameInfo.Worlds.WORLDSIZE_SMALL.ID] = 13,		-- MOD: 28 - ( 6 regional + 1 marble + 3 city-state + 5 random) = 13
		[GameInfo.Worlds.WORLDSIZE_STANDARD.ID] = 11,	-- MOD: 28 - ( 8 regional + 1 marble + 3 city-state + 5 random) = 11
		[GameInfo.Worlds.WORLDSIZE_LARGE.ID] = 9,		-- MOD: 28 - (10 regional + 1 marble + 3 city-state + 5 random) =  9
		[GameInfo.Worlds.WORLDSIZE_HUGE.ID] = 6,		-- MOD: 28 - (12 regional + 1 marble + 3 city-state + 6 random) =  6
		}
...
function AssignStartingPlots:AssignLuxuryRoles()
	...
	-- MOD: Modified this so that increasing the civ count on maps below the maximum Regional luxury total (which is now all of them) won't subtract from the Random total first.  
	-- MOD: Instead, it'll subtract from the Disabled total first.  This can be seen in the default game with maps smaller than Standard.
	--[[	MOD: Disabled -- old method
	...
	local worldsizes = {																-- MOD: New -- this ensures that the total luxuries used match the default game, except for Huge, which really needed a few more anyway!
	...
	self.iNumTypesRandom = worldsizes[Map.GetWorldSize()]								-- MOD: New
	self.iNumTypesDisabled = self:GetDisabledLuxuriesTargetNumber()						-- MOD: Updated
	if self.iNumTypesUnassigned < self.iNumTypesRandom + self.iNumTypesDisabled then	-- MOD: New -- new loop
		self.iNumTypesDisabled = self.iNumTypesUnassigned - self.iNumTypesRandom
		if self.iNumTypesDisabled < 0 then
			self.iNumTypesDisabled = 0
		end
	end
	if self.iNumTypesUnassigned < self.iNumTypesRandom then								-- MOD: New -- new loop
		self.iNumTypesRandom = self.iNumTypesUnassigned
	else
		self.iNumTypesRandom = self.iNumTypesUnassigned - self.iNumTypesDisabled
	end
	...
	local countdown = math.min(self.iNumTypesUnassigned, self.iNumTypesDisabled);		-- MOD: Updated -- using self.iNumTypesDisabled instead of maxToDisable
	...
	print("-"); print("Total unique regional luxuries: ", self.iNumTypesAssignedToRegions);		-- MOD: New -- I just added this for easier debugging and some other tests.
		...
		for direction_index = 1, 6 do
			for plot_to_handle = 1, ripple_radius do
				...
				elseif wrapY == false and (nextY < 0 or nextY >= iH) then
					...
					local featureType = plot:GetFeatureType()
					local plotIndex = realY * iW + realX + 1;
					if plot:GetResourceType(-1) == -1 then	-- MOD: Fixed -- Check to make sure this plot doesn't already contain a resource!
																		  -- This corrects a rare bug that occurs and denies some civs their 2nd bonus luxury type, since it would mark a tile's resource options to "true" when there's already a regional luxury present.
							...
							if terrainType == TerrainTypes.TERRAIN_COAST then
								if plot:IsLake() == false then
									if featureType ~= self.feature_atoll and featureType ~= FeatureTypes.FEATURE_ICE then
										...
										allowed_luxuries[self.crab_ID] = true;	-- MOD: Fixed -- was missing per GetIndicesForLuxuryType() 
																							  -- these fixes help city-state luxuries and 2nd-bonus luxuries at city starts to be consistent with regional and random luxury placements.
									end
								end
							end
						...
						elseif plotType == PlotTypes.PLOT_HILLS and terrainType ~= TerrainTypes.TERRAIN_SNOW then
							...
							allowed_luxuries[self.copper_ID] = true;					-- MOD: Fixed -- was missing per GetIndicesForLuxuryType()
							allowed_luxuries[self.gems_ID] = true;
							allowed_luxuries[self.jade_ID] = true;						-- MOD: New
							allowed_luxuries[self.amber_ID] = true;						-- MOD: New
							if featureType == FeatureTypes.NO_FEATURE then
								allowed_luxuries[self.marble_ID] = true;
							else
								allowed_luxuries[self.truffles_ID] = true;				-- MOD: Fixed -- was missing per GetIndicesForLuxuryType()
								if featureType == FeatureTypes.FEATURE_JUNGLE then
									allowed_luxuries[self.citrus_ID] = true;			-- MOD: Fixed -- was missing per GetIndicesForLuxuryType()
								end
							end
						elseif plotType == PlotTypes.PLOT_LAND then
							if featureType == FeatureTypes.NO_FEATURE then
								if terrainType == TerrainTypes.TERRAIN_TUNDRA then
									...
									allowed_luxuries[self.silver_ID] = true;
									allowed_luxuries[self.gold_ID] = true;				-- MOD: Added -- now available in tundra too
									allowed_luxuries[self.marble_ID] = true; 			-- MOD: (with default code, this would've been disabled as it should not exist per the default GetIndicesForLuxuryType().  One of the new plot lists added corrects this.)
									allowed_luxuries[self.copper_ID] = true;			-- MOD: Fixed -- was missing per GetIndicesForLuxuryType()
									allowed_luxuries[self.salt_ID] = true;				-- MOD: Fixed -- was missing per GetIndicesForLuxuryType()
									allowed_luxuries[self.amber_ID] = true;				-- MOD: New
								elseif terrainType == TerrainTypes.TERRAIN_DESERT then
									allowed_luxuries[self.gold_ID] = true;
									allowed_luxuries[self.marble_ID] = true;
									allowed_luxuries[self.incense_ID] = true;
									allowed_luxuries[self.copper_ID] = true;			-- MOD: Added -- now available in desert too
									allowed_luxuries[self.silver_ID] = true;			-- MOD: Added -- now available in desert too
									allowed_luxuries[self.salt_ID] = true;				-- MOD: Fixed -- was missing per GetIndicesForLuxuryType()
								elseif terrainType == TerrainTypes.TERRAIN_PLAINS then
									allowed_luxuries[self.ivory_ID] = true;
									allowed_luxuries[self.wine_ID] = true;
									allowed_luxuries[self.incense_ID] = true;
									allowed_luxuries[self.coffee_ID] = true;			-- MOD: New
									allowed_luxuries[self.tea_ID] = true;				-- MOD: New
									allowed_luxuries[self.tobacco_ID] = true;			-- MOD: New
									allowed_luxuries[self.olives_ID] = true;			-- MOD: New
									allowed_luxuries[self.perfume_ID] = true;			-- MOD: New
									if plot:IsFreshWater() then							-- MOD: New -- added fresh water check
										allowed_luxuries[self.cotton_ID] = true;		-- MOD: New -- now is also placed on fresh water flat plains								
									else
										allowed_luxuries[self.gold_ID] = true;			-- MOD: New -- now is also placed on dry flat plains
										allowed_luxuries[self.marble_ID] = true;		-- MOD: Updated -- now requires dry plains, instead of any, just like grass
										allowed_luxuries[self.salt_ID] = true;			-- MOD: Updated -- (with default code, it would be any plains: Fixed -- was missing per GetIndicesForLuxuryType())
									end
								elseif terrainType == TerrainTypes.TERRAIN_GRASS then
									allowed_luxuries[self.cotton_ID] = true;
									allowed_luxuries[self.wine_ID] = true;
									allowed_luxuries[self.ivory_ID] = true;				-- MOD: Updated -- now is allowed on fresh water grass too
									allowed_luxuries[self.coffee_ID] = true;			-- MOD: New
									allowed_luxuries[self.tea_ID] = true;				-- MOD: New
									allowed_luxuries[self.tobacco_ID] = true;			-- MOD: New
									allowed_luxuries[self.perfume_ID] = true;			-- MOD: New
									if plot:IsFreshWater() then
										...
										allowed_luxuries[self.marble_ID] = true;
										allowed_luxuries[self.silver_ID] = true;		-- MOD: Fixed -- was missing per GetIndicesForLuxuryType()
										allowed_luxuries[self.copper_ID] = true;		-- MOD: Fixed -- was missing per GetIndicesForLuxuryType()
										allowed_luxuries[self.olives_ID] = true;		-- MOD: New
									end
								end
							elseif featureType == FeatureTypes.FEATURE_MARSH then		
								...
								allowed_luxuries[self.sugar_ID] = true;
								allowed_luxuries[self.truffles_ID] = true;				-- MOD: Fixed -- was missing per GetIndicesForLuxuryType()
								allowed_luxuries[self.spices_ID] = true;				-- MOD: Fixed -- was missing per GetIndicesForLuxuryType()
							elseif featureType == FeatureTypes.FEATURE_FLOOD_PLAINS then		
								allowed_luxuries[self.cotton_ID] = true;
								allowed_luxuries[self.incense_ID] = true;
								allowed_luxuries[self.citrus_ID] = true;				-- MOD: Fixed -- was missing per GetIndicesForLuxuryType()
								allowed_luxuries[self.sugar_ID] = true;					-- MOD: Fixed -- was missing per GetIndicesForLuxuryType()
								allowed_luxuries[self.olives_ID] = true;				-- MOD: New
							elseif featureType == FeatureTypes.FEATURE_JUNGLE then		
								allowed_luxuries[self.gems_ID] = true;
								allowed_luxuries[self.dye_ID] = true;
								...
								allowed_luxuries[self.citrus_ID] = true;				-- MOD: Fixed -- was missing per GetIndicesForLuxuryType()
								allowed_luxuries[self.truffles_ID] = true;				-- MOD: Fixed -- was missing per GetIndicesForLuxuryType()
								allowed_luxuries[self.cocoa_ID] = true;					-- MOD: New
								allowed_luxuries[self.jade_ID] = true;					-- MOD: New
							elseif featureType == FeatureTypes.FEATURE_FOREST then		
								allowed_luxuries[self.fur_ID] = true;
								allowed_luxuries[self.dye_ID] = true;
								allowed_luxuries[self.salt_ID] = true;					-- MOD: Fixed -- was missing per GetIndicesForLuxuryType()
								allowed_luxuries[self.amber_ID] = true;					-- MOD: New
								if terrainType ~= TerrainTypes.TERRAIN_TUNDRA then		-- MOD: Updated -- removed "else" and change to "~=" not equal
									allowed_luxuries[self.spices_ID] = true;			-- MOD: (with default code, copper would be added when forest terrain "is" tundra)
									allowed_luxuries[self.silk_ID] = true;
									allowed_luxuries[self.citrus_ID] = true;			-- MOD: Fixed -- was missing per GetIndicesForLuxuryType()
									allowed_luxuries[self.truffles_ID] = true;			-- MOD: Fixed -- was missing per GetIndicesForLuxuryType()
									allowed_luxuries[self.cocoa_ID] = true;				-- MOD: New
...
function AssignStartingPlots:GenerateLuxuryPlotListsAtCitySite(x, y, radius, bRemoveFeatureIce)
	...
	local region_dry_plains_flat_no_feature, region_fresh_water_plains_flat_no_feature = {}, {};	-- MOD: New
	local region_desert_or_tundra_flat_no_feature, region_grass_flat_no_feature = {}, {};			-- MOD: New
	local region_tundra_flat_no_feature, region_tundra_flat_forest = {}, {};						-- MOD: New
					...
					elseif wrapY == false and (nextY < 0 or nextY >= iH) then
						...
						-- Otherwise generate the plot lists.
							...
							elseif plotType == PlotTypes.PLOT_LAND then
								if featureType == FeatureTypes.NO_FEATURE then
									if terrainType == TerrainTypes.TERRAIN_TUNDRA then
										table.insert(region_tundra_flat_no_feature, plotIndex);						-- MOD: New
										table.insert(region_desert_or_tundra_flat_no_feature, plotIndex);			-- MOD: New
									...
									elseif terrainType == TerrainTypes.TERRAIN_PLAINS then
										...
										if plot:IsFreshWater() then													-- MOD: New -- fresh water check
											table.insert(region_fresh_water_plains_flat_no_feature, plotIndex);		-- MOD: New
										else
											table.insert(region_dry_plains_flat_no_feature, plotIndex);				-- MOD: New
										end
									elseif terrainType == TerrainTypes.TERRAIN_GRASS then
										table.insert(region_grass_flat_no_feature, plotIndex);						-- MOD: New
										if plot:IsFreshWater() then
								...
								elseif featureType == FeatureTypes.FEATURE_FOREST then		
									...
									if terrainType == TerrainTypes.TERRAIN_TUNDRA then
										table.insert(region_tundra_flat_including_forests, plotIndex);
										table.insert(region_tundra_flat_forest, plotIndex);							-- MOD: New
									else
	...
	region_grass_flat_no_feature,				-- 16	-- MOD: New
	region_tundra_flat_no_feature,				-- 17	-- MOD: New
	region_dry_plains_flat_no_feature,			-- 18	-- MOD: New
	region_fresh_water_plains_flat_no_feature,	-- 19	-- MOD: New
	region_desert_or_tundra_flat_no_feature,	-- 20	-- MOD: New
	region_tundra_flat_forest,					-- 21	-- MOD: New
	};
...
function AssignStartingPlots:GetIndicesForLuxuryType(resource_ID)
	...
	local primary, secondary, tertiary, quaternary = -1, -1, -1, -1;
																	-- MOD: Modders, don't forget, if you add or remove indices, make sure you have the correct number of primary, secondary, etc. as well!
	if resource_ID == self.whale_ID then
		primary = 1;
	elseif resource_ID == self.pearls_ID then
		primary = 1;
	elseif resource_ID == self.gold_ID then
		primary, secondary, tertiary, quaternary = 4, 5, 20, 18;	-- MOD: Updated -- original = 4, 10, 5	-- added dry plains and flat open tundra
	elseif resource_ID == self.silver_ID then
		primary, secondary, tertiary, quaternary = 4, 5, 20, 12;	-- MOD: Updated -- original = 4, 5, 14, 12	-- leave flat tundra forests for other luxuries which need it more, also added flat desert
	elseif resource_ID == self.gems_ID then
		primary, secondary, tertiary, quaternary = 6, 7, 4, 8;
	elseif resource_ID == self.marble_ID then
		primary, secondary, tertiary, quaternary = 12, 18, 20, 4;	-- MOD: Updated -- original = 12, 10, 11, 4	-- this allows it on flat featureless tundra along with the other 4 types
	elseif resource_ID == self.ivory_ID then
		primary, secondary, tertiary, quaternary = 18, 12, 19, 13;	-- MOD: Updated -- original = 11, 12 -- made this a little more flexible on grass, but both plains and grass will prioritize dry versions first
	elseif resource_ID == self.fur_ID then
		primary, secondary, tertiary = 21, 17, 15;					-- MOD: Updated -- original = 14, 15 -- favors tundra forests first before open tundra instead of together; tundra forests are much better for this camp luxury
	...
	elseif resource_ID == self.sugar_ID then
		primary, secondary, tertiary, quaternary = 2, 8, 13, 3;		-- MOD: Updated -- original = 2, 8, 3, 13 -- moved flood plains to least priority to reduce chances of excessive clumping along a flood plain river.
	elseif resource_ID == self.cotton_ID then
		primary, secondary, tertiary, quaternary = 13, 12, 19, 3;	-- MOD: Updated -- original = 3, 13, 12 -- moved flood plains to least priority to avoid too much clumping, added fresh water plains for more flexibility.
	elseif resource_ID == self.wine_ID then
		primary, secondary, tertiary, quaternary = 19, 18, 12, 13;	-- MOD: Updated -- original = 11, 12, 13 -- now favors wet plains before dry plains, instead of lumping them together
	elseif resource_ID == self.incense_ID then
		primary, secondary, tertiary, quaternary = 10, 3, 18, 19;	-- MOD: Updated -- original = 10, 3, 11 -- now favors dry plains before wet plains (as it does with desert), instead of lumping them together
	elseif resource_ID == self.copper_ID then
		primary, secondary, tertiary, quaternary = 4, 5, 12, 20;	-- MOD: Updated -- original = 4, 5, 12, 14	-- now favors flat open tundra instead of also including all forests, along with desert
	elseif resource_ID == self.salt_ID then
		primary, secondary, tertiary, quaternary = 18, 10, 17, 9;	-- MOD: Updated -- original = 11, 10, 14, 9	-- quaternary type already covers forests as a backup, so favor flat featureless tundra (which could use food); also favor dry plains
	...
	elseif resource_ID == self.coffee_ID then						
		primary, secondary, tertiary, quaternary = 19, 18, 12, 13;	-- MOD: New	-- favors plains (fresh water first), but will also be placed in grass (dry first)
	elseif resource_ID == self.cocoa_ID then						
		primary, secondary, tertiary = 8, 6, 15;					-- MOD: New	-- favors jungles but will also be placed in non-tundra forests for flexibility, similar to citrus
	elseif resource_ID == self.tea_ID then							
		primary, secondary, tertiary, quaternary = 13, 12, 19, 18;	-- MOD: New	-- favors grasslands (fresh water first), but will also be placed in plains (fresh water first)
	elseif resource_ID == self.tobacco_ID then						
		primary, secondary, tertiary, quaternary = 18, 19, 12, 13;	-- MOD: New	-- favors plains (dry ones first) and then grass (dry ones first)
	elseif resource_ID == self.amber_ID then						
		primary, secondary, tertiary, quaternary = 17, 9, 5, 4;		-- MOD: New	-- favors open tundra first, then flat forest tiles, and lastly all hills (covered ones first)
	elseif resource_ID == self.jade_ID then							
		primary, secondary, tertiary, quaternary = 4, 6, 7, 8;		-- MOD: New	-- favors hills (open first), then covered hills (jungle first), and lastly flat jungles
	elseif resource_ID == self.olives_ID then						
		primary, secondary, tertiary, quaternary = 18, 19, 12, 3;	-- MOD: New -- favors plains (dry ones first), then dry grass, and lastly flood plains
...
function AssignStartingPlots:GetRegionLuxuryTargetNumbers()
	...
	-- MOD: Updated -- increased inital value when increasing total civ count by 2.  Instead of decreasing by 2, it'll decrease copies of regional luxuries placed by 1.
	-- MOD: Rough Example -- Standard 8 civs x 6 copies of each regional luxury = 48 ... 10 x 4 = 40 ... 10 x 5 = 50 ... 50 is closer to 48 than 40
	-- MOD: This will not hurt the random luxury total to be placed since it always places a minimum number at least.  
	local duel_values = table.fill(1, 22); -- Max is one per region for all player counts at this size.
	--
	--[[	MOD: Disabled -- old values
...
function AssignStartingPlots:PrintFinalResourceTotalsToLog()
	...
	print("- Mod LUXURY Resources -");												-- MOD: New
	print("- Cocoa...: ", self.amounts_of_resources_placed[self.cocoa_ID + 1]);		-- MOD: New
	print("- Coffee..: ", self.amounts_of_resources_placed[self.coffee_ID + 1]);	-- MOD: New
	print("- Tea.....: ", self.amounts_of_resources_placed[self.tea_ID + 1]);		-- MOD: New
	print("- Tobacco.: ", self.amounts_of_resources_placed[self.tobacco_ID + 1]);	-- MOD: New
	print("- Amber...: ", self.amounts_of_resources_placed[self.amber_ID + 1]);		-- MOD: New
	print("- Jade....: ", self.amounts_of_resources_placed[self.jade_ID + 1]);		-- MOD: New
	print("- Olives..: ", self.amounts_of_resources_placed[self.olives_ID + 1]);	-- MOD: New
	print("- Perfume.: ", self.amounts_of_resources_placed[self.perfume_ID + 1]);	-- MOD: New
	print("-");
